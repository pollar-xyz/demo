...
const OPS: Op[] = [ 'create_account', 'payment', 'path_payment_strict_send', 'change_trust', 'invoke_contract' ];
...
function PathInput({ value, onChange }: { value: Asset[]; onChange: (v: Asset[]) => void }) {
  ...
  return (
    <Field label={tr.transactions.intermediatePath} note={tr.transactions.intermediatePathNote}>
      <div className="space-y-3">
        {value.map((asset, i) => (
        ...
        <button type="button" onClick={add} className={btn('secondary')}>{tr.transactions.addAsset}</button>
      </div>
    </Field>
  );
}
...
export default function TransactionsPage() {
  ...
  async function fetchMethods() {
    ...
    try {
      ...
      setIcFetchError(e instanceof Error ? e.message : tr.transactions.fetchFailed);
    } finally {
      setIcFetching(false);
    }
  }
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <Select
              ...
              options={OPS.map(value => ({ value, label: tr.transactions.ops[value] }))}
            />
          </div>
          ...
          {op === 'create_account' && (
            <div className="space-y-4">
              ...
              <Field label={tr.transactions.startingBalance} required note={tr.transactions.startingBalanceNote}>
                <input
                  className={inp}
          ...
          {op === 'payment' && (
            <div className="space-y-4">
              ...
              <AssetInput label={tr.transactions.asset} required value={pyAsset} onChange={setPyAsset} />
          ...
          {op === 'path_payment_strict_send' && (
            <div className="space-y-4">
              ...
              />
              <Field label={tr.transactions.destMin} required note={tr.transactions.destMinNote}>
                <input
                  className={inp}
                  value={ppDestMin}
          ...
          {op === 'change_trust' && (
            <div className="space-y-4">
              ...
              <Field label={tr.transactions.trustLimit} note={tr.transactions.trustLimitNote}>
                <input
                  className={inp}
                  value={ctLimit}
            ...
            <div className="space-y-4">
              <Field label={tr.transactions.contractId} required>
                <div className="flex flex-col gap-2 sm:flex-row">
                  ...
                  >
                    {icFetching ? tr.transactions.fetching : tr.transactions.fetchMethods}
                  </button>
                </div>
          ...
          <div className="space-y-3 pt-2 border-t border-border">
            <p className="text-xs font-mono text-muted-light">{tr.transactions.options}</p>
            <Field label={tr.transactions.timeout} note={tr.transactions.timeoutNote}>
              <input
                className={inp}
                type="number"
                value={optTimeoutSec}
            ...
            <Field label={tr.transactions.maxFee} note={tr.transactions.maxFeeNote}>
              <input
                className={inp}
                type="number"
            ...
            <div className="space-y-1">
              <label className={lbl}>{tr.transactions.memo} <span className="ml-1 text-muted-light">{tr.common.optional}</span></label>
              <div className="flex gap-2 mb-2 flex-wrap">
              ...
              {optMemoType !== 'none' && (
                <input
                  ...
                  placeholder={optMemoType === 'id' ? tr.transactions.memoPhId : tr.transactions.memoPhText}
                  spellCheck={false}
                />
              )}
            </div>
          ...
          <div className="space-y-2 pt-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                {!isAuthenticated ? tr.common.connectWalletToContinue
                  : tx.step === 'building' ? tr.transactions.building
                  : '1. buildTx'}
              </button>
            ...
            <p className="text-[10px] font-mono text-muted-light">
              {tr.transactions.tipPre}<code className="text-foreground">openTxModal()</code>{tr.transactions.tipPost}
            </p>
          </div>
          ...
          <div className="rounded-lg border border-border overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-border bg-surface">
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono text-muted-light">{tr.transactions.stateLabel}</span>
                <span
            ...
            <div className="p-4 space-y-3 bg-background min-h-16">
              {tx.step === 'idle' && (
                <p className="text-xs font-mono text-muted-light">{tr.transactions.stateIdle}</p>
              )}
