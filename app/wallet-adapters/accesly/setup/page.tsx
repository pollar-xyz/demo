...
const INSTALL_CODE = "pnpm add @pollar/accesly-adapter @accesly/react";

const CORE_CODE = `import { PollarClient } from '@pollar/core';
import { createAcceslyAdapter } from '@pollar/accesly-adapter';

// 'address' = the user's Accesly Smart Account (C-address).
// 'signXdr' signs a full transaction XDR via Accesly, returning the signed XDR.
const accesly = createAcceslyAdapter({
  address,
  signXdr: async (xdr) => signWithAccesly(xdr),
});
...
// Sign in with the Accesly smart account (id === adapter.type):
client.login({ provider: 'accesly' });`;

const REACT_CODE = `import { useMemo } from 'react';
...
export default function AcceslyAdapterPage() {
  const { locale } = useI18n();
  const tt = acceslyDict[locale];
